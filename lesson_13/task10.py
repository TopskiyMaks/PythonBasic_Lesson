...
def ras_credit(credit_sum, year, procent):
    S = float(int(credit_sum.split("e")[0]) * 10**int((credit_sum.split("e")[1])))
    i = procent/100
    n = year
    K = i * (1 + i) ** n / ((1 + i) ** n - 1)
    A = K * S
    ...
    for k in range(3):
        print(f'Период: {k + 1}\n')
        procents_all = ost * i
        body_credit = A - procents_all
        print(f'Остаток долга на начало периода: {ost}')
        print(f'Выплачено процентов: {procents_all}')
        print(f'Выплачено тела кредита: {body_credit}\n')
        ost -= body_credit
    print('Остаток долга:', ost)
...
if __name__ == '__main__':
    # S = input('Введите сумму кредита: ')
    # year = int(input('На сколько лет?: '))
    # procent = int(input('Сколько процентов годовых? '))
    S = '40e6'
    year = 5
    procent = 6
    ras_credit(S, year, procent)
